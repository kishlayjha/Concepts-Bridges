...
def main():
    ...
    learnGlobalTransformationMatrix_M = classGlobaltransformationMatrixInit.globalTransformationMatrix()
    classGlobaltransformationMatrixInit.generateTransformationMatrix()
    print("learnGlobalTransformationMatrix_M: ",learnGlobalTransformationMatrix_M.shape)

    anchorPairsYearOne = classGlobaltransformationMatrixInit.loadLocalAnchorPairs(settings.dict['yearBase'])
    # print(len(anchorPairsYearOne))

    anchorPairsYearTwo = classGlobaltransformationMatrixInit.loadLocalAnchorPairs(settings.dict['yearTarget'])
    # print(len(anchorPairsYearTwo))

    commonAnchorPairs = []
    for counter, termPair in enumerate(anchorPairsYearOne):
        if termPair in anchorPairsYearTwo:
            commonAnchorPairs.append(termPair)

    # print(len(commonAnchorPairs))

    dataMatrix_Base_Year = classGlobaltransformationMatrixInit.prepareDataMarixOfLocalAnchorPairs(commonAnchorPairs, settings.dict['yearBase'])
    dataMatrix_Target_Year = classGlobaltransformationMatrixInit.prepareDataMarixOfLocalAnchorPairs(commonAnchorPairs,settings.dict['yearTarget'])
    learnLocalTransformationMatrix = classGlobaltransformationMatrixInit.localTransformationMatrix(dataMatrix_Base_Year, dataMatrix_Target_Year)
    print("Local TransformationMatrix: ",learnLocalTransformationMatrix.shape)

    finalTransformationMatrix = np.add(0.40*learnGlobalTransformationMatrix_M, 0.60*learnLocalTransformationMatrix)
    # finalTransformationMatrix = np.add(learnGlobalTransformationMatrix_M)
    print("Final TransformationMatrix: ", finalTransformationMatrix.shape)
    ...
    previousYearBTerms = []
    for counter, index in enumerate(open(settings.dict['FILES_PATH'] + os.sep + "candidateBTerms" + os.sep + settings.dict['inputTerm1'] + "-" + settings.dict['inputTerm2'] +"-candidateBTerms")):
        index = index.strip()
        previousYearBTerms.append(index.strip().lower())
    ...
    for counter, item in enumerate(previousYearBTerms):
        ...
        if (item in startInputTermNearestNeighbor or item in endInputTermNearestNeighbor):
            ...
            bTermCosineMatrix[counter] = np.dot(embedding1, finalTransformationMatrix)
            # path = pathlib.Path(
    ...
    for key, value in sorted_BTerms:
        meshTerm = key
        count = value
        folderName = settings.dict['inputTerm1']+"-"+settings.dict['inputTerm2']
        with open(settings.dict['FILES_PATH'] + os.sep + "baselines" + os.sep+  "KDD" + os.sep + folderName + os.sep + "global-and-local-final-sortedBTermsByCosine.txt", 'a') as the_file:
            prepareString = meshTerm + "\t" + str(count)
            the_file.write(prepareString)
            the_file.write("\n")
    print("writing of global and local transformation matrix completed")
