...
def main():
    ...
    print(learnLocalTransformationMatrix.shape)
    ...
    previousYearBTerms = []
    for counter, item in enumerate(open(settings.dict['FILES_PATH'] + os.sep + "candidateBTerms" + os.sep +
                                                settings.dict['inputTerm1'] + "-" + settings.dict[
        'inputTerm2'] + "-candidateBTerms")):
    ...
    for counter, item in enumerate(previousYearBTerms):
        ...
        if (item in startInputTermNearestNeighbor or item in endInputTermNearestNeighbor):
            ...
            bTermCosineMatrix[counter] = np.dot(embedding1, learnLocalTransformationMatrix)
            # path = pathlib.Path(
    ...
    for key, value in sorted_BTerms:
        meshTerm = key
        count = value
        with open(settings.dict['FILES_PATH'] + os.sep + settings.dict[
            'year'] + os.sep + "local-trans-final-sortedBTermsByCosine.txt", 'a') as the_file:
            prepareString = meshTerm + "\t" + str(count)
            the_file.write(prepareString)
            the_file.write("\n")
    print("writing of local transformation matrix completed")
